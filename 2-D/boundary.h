...
void Mur_1st_ABC(int id){
  for(i = 1 + high_S; i <= high_E; ++i){
    if(Process_Point_X == 0)
      P1[0][i] = PX1[1][i] + Ca0 * (P1[1][i] - PX1[0][i]);
    if(Process_Point_X == wd-1)
      P1[x_max-1][i] = PX1[4][i] + Ca0 * (P1[x_max-2][i] - PX1[3][i]);
  }
  for(i = 1 + wide_S; i <= wide_E; ++i){
    if(Process_Point_Y==0)
      P1[i][0] = PY1[1][i] + Ca0 * (P1[i][1] - PY1[0][i]);
    if(Process_Point_Y==hg-1)
      P1[i][y_max-1] = PY1[4][i] + Ca0 * (P1[i][y_max-2] - PY1[3][i]);
  }
}
